...
data = readtable('error_differences.csv');  % Using readtable to load the CSV as a table

% Extract the 'Difference' column from the table
differences = data.Difference;  % Assuming 'Difference' column is present in the CSV

% Compute mean and standard deviation of the differences data
mu = mean(differences);  % Mean of the data
sigma = std(differences);  % Standard deviation of the data

% Generate points for the Gaussian distribution curve based on the data
x = linspace(min(differences) - 3*sigma, max(differences) + 3*sigma, 100);  % Create 100 points for the plot, extended beyond data range
...
% Dynamically set y-axis limits based on the peak of the Gaussian distribution
y_max = max(y_data_fit);  % Get the maximum value of the Gaussian curve
ylim([0, y_max + 0.05 * y_max]);  % Set the y-axis limit to slightly exceed the peak for better visualization

% Ensure the plot limits include the full range of the data and the Gaussian curve
xlim([min(differences) - 3*sigma, max(differences) + 3*sigma]);  % Extend the x-axis beyond the data range

% Display the mean and standard deviation on the plot
text(mu, y_max * 0.9, ['\mu = ', num2str(mu), ', \sigma = ', num2str(sigma)], ...
